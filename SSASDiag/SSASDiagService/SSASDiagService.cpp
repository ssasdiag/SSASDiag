...
VOID WINAPI SSASDiagServiceHandler( DWORD fdwControl );
...
{ 
	{pServiceName, SSASDiagServiceMain}, 
	{NULL, NULL}
};
...
VOID WINAPI SSASDiagServiceMain( DWORD dwArgc, LPTSTR *lpszArgv )
{
    ...
    hServiceStatusHandle = RegisterServiceCtrlHandler(pServiceName, SSASDiagServiceHandler); 
...
{  
	...
	else
	{
		...
		else
		{
			...
			sd.lpDescription = pServiceDesc;
			char pTemp[121];
			BOOL bResult = ChangeServiceConfig2(schService, SERVICE_CONFIG_DESCRIPTION, &sd);			
