    ...
    public partial class frmSSASDiag : Form
    {
        ...
        private void ProfilerAnalysisContextMenu_Click(object sender, EventArgs e)
        {
            ...
            if ((sender as MenuItem).Text == "Find all queries/commands overlapping with selection")
            {
                ...
                {
                    if (row < 0)
                        strQry += (strQry == "" ? ("select a.RowNumber, a.Duration, a.EventClass, a.EventClassName, a.CurrentTime, a.StartTime, a.ConnectionID, a.NTUserName, a.NTDomainName, a.DatabaseName, a.TextData, a.ClientProcessID, a.ApplicationName, a.CPUTime, a.EventSubclass, a.SPID, convert(nvarchar(4000), a.RequestParameters), convert(nvarchar(4000), a.RequestProperties) from [" + AnalysisTraceID + "_v] a, (select * from[" + AnalysisTraceID + "_QueriesAndCommandsIncludingIncomplete] where StartRow > " + -row + " or EndRow > " + -row + " or EndRow is null and StartRow <> " + -row + ") b where (b.EndRow is null and a.RowNumber = b.StartRow) or(not b.EndRow is null and a.RowNumber = b.EndRow)")
                        ...
                        strQry += (strQry == "" ? ("select a.RowNumber, a.Duration, a.EventClass, a.EventClassName, a.CurrentTime, a.StartTime, a.ConnectionID, a.NTUserName, a.NTDomainName, a.DatabaseName, a.TextData, a.ClientProcessID, a.ApplicationName, a.CPUTime, a.EventSubclass, a.SPID, convert(nvarchar(4000), a.RequestParameters), convert(nvarchar(4000), a.RequestProperties) from[" + AnalysisTraceID + "_v] a, (select StartTime from[" + AnalysisTraceID + "] where RowNumber > " + row + ") b where a.eventclass in (10, 16) and a.CurrentTime >= b.StartTime").Replace("[Table", "[" + AnalysisTraceID)
                                 : ("\r\nunion\r\nselect a.RowNumber, a.Duration, a.EventClass, a.EventClassName, a.CurrentTime, a.StartTime, a.ConnectionID, a.NTUserName, a.NTDomainName, a.DatabaseName, a.TextData, a.ClientProcessID, a.ApplicationName, a.CPUTime, a.EventSubclass, a.SPID, convert(nvarchar(4000), a.RequestParameters), convert(nvarchar(4000), a.RequestProperties) from[" + AnalysisTraceID + "_v] a, (select StartTime from[" + AnalysisTraceID + "] where RowNumber > " + row + ") b where a.eventclass in (10, 16) and a.CurrentTime >= b.StartTime").Replace("[Table", "[" + AnalysisTraceID));
                    else
                        strQry += (strQry == "" ? ("select a.RowNumber, a.Duration, a.EventClass, a.EventClassName, a.CurrentTime, a.StartTime, a.ConnectionID, a.NTUserName, a.NTDomainName, a.DatabaseName, a.TextData, a.ClientProcessID, a.ApplicationName, a.CPUTime, a.EventSubclass, a.SPID, convert(nvarchar(4000), a.RequestParameters), convert(nvarchar(4000), a.RequestProperties)\r\nfrom [" + AnalysisTraceID + "_v] a,\r\n(select StartTime, CurrentTime from [" + AnalysisTraceID + "] where RowNumber = " + row + ") b\r\nwhere a.eventclass in (10, 16)\r\nand a.CurrentTime >= b.StartTime and a.CurrentTime <= b.CurrentTime").Replace("[Table", "[" + AnalysisTraceID)
                                 : ("\r\nunion\r\nselect a.RowNumber, a.Duration, a.EventClass, a.EventClassName, a.CurrentTime, a.StartTime, a.ConnectionID, a.NTUserName, a.NTDomainName, a.DatabaseName, a.TextData, a.ClientProcessID, a.ApplicationName, a.CPUTime, a.EventSubclass, a.SPID, convert(nvarchar(4000), a.RequestParameters), convert(nvarchar(4000), a.RequestProperties)\r\nfrom [" + AnalysisTraceID + "_v] a,\r\n(select StartTime, CurrentTime from [" + AnalysisTraceID + "] where RowNumber = " + row + ") b\r\nwhere a.eventclass in (10, 16)\r\nand a.CurrentTime >= b.StartTime and a.CurrentTime <= b.CurrentTime").Replace("[Table", "[" + AnalysisTraceID));
                }
            ...
            else if ((sender as MenuItem).Text == "Lookup all the specific executions of this request")
            {
                ...
                {
                    string strBase = "";
                    strBase = "select * from [" + AnalysisTraceID + "_v] where EventClass in (10,16) and convert(nvarchar(4000), TextData) = (select convert(nvarchar(4000), TextData) from [" + AnalysisTraceID + "] where RowNumber = " + row + ")";
                    strQry += (strQry == "" ? strBase : "\r\nunion\r\n" + strBase);
                }
                txtProfilerAnalysisQuery.Text = strQry + "\r\norder by RowNumber";
                ExecuteProfilerAnalysisDrillthroughContextQuery();
