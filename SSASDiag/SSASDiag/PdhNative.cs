...
namespace PdhNative
{
    ...
    public static class PdhLogOpenOption
    {
    ...
    public class PdhHelper : IDisposable
    {
        ...
        public static extern uint PdhUpdateLog([In] PdhSafeLogHandle phLog, [In] string szUserString);
        ...
        internal sealed class PdhSafeDataSourceHandle : SafeHandle
        {
            ...
            {
                return (PdhCloseLog(base.handle, 0) == 0);
            }
        ...
        public class PdhSafeLogHandle : SafeHandleZeroOrMinusOneIsInvalid
        {
            public PdhSafeLogHandle() : base(true) { }
            [ReliabilityContract(Consistency.WillNotCorruptState, Cer.MayFail)]
            protected override bool ReleaseHandle()
            { return PdhCloseLog(handle, PDH_FLAGS_CLOSE_QUERY) == 0; }
        }

        [SecurityPermission(SecurityAction.LinkDemand, UnmanagedCode = true)]
        public class PdhSafeQueryHandle : SafeHandleZeroOrMinusOneIsInvalid
        {
            public PdhSafeQueryHandle() : base(true) { }
            [ReliabilityContract(Consistency.WillNotCorruptState, Cer.MayFail)]
            protected override bool ReleaseHandle()
        ...
        [SecurityPermission(SecurityAction.LinkDemand, UnmanagedCode = true)]
        public class PdhSafeCounterHandle : SafeHandleZeroOrMinusOneIsInvalid
        {
            public PdhSafeCounterHandle() : base(true) { }
            [ReliabilityContract(Consistency.WillNotCorruptState, Cer.MayFail)]
            protected override bool ReleaseHandle()
            { return PdhRemoveCounter(handle) == 0; }
        }
        /*
        //
        ...
        [DllImport("pdh.dll", CharSet = CharSet.Unicode)]
        public static extern uint PdhConnectMachine(string szMachineName);
        ...
        [DllImport("Pdh.dll")]
        static extern uint PdhOpenQueryW([MarshalAs(UnmanagedType.LPWStr)] string szDataSource, IntPtr dwUserData, out PdhSafeQueryHandle phQuery);
        ...
        [DllImport("pdh.dll", CharSet = CharSet.Unicode)]
        private static extern uint PdhAddEnglishCounter(PdhSafeQueryHandle queryHandle, string counterPath, IntPtr userData, out IntPtr counterHandle);
        ...
        public uint ConnectMachine(string MachineName)
        {
            return PdhConnectMachine(MachineName);
        }

        public uint OpenQuery()
        {
            if (_hDataSource != null)
                return PdhOpenQueryH(_hDataSource, IntPtr.Zero, out _hQuery);
            else
        ...
        public void UpdateLog(string UserString)
        {
            Debug.Assert(_hOutputLog != null);

            uint res = PdhUpdateLog(_hOutputLog, UserString);
        }
        ...
        public uint AddCounters(ref StringCollection validPaths, bool bFlushOldCounters)
        {
            ...
            foreach (string counterPath in validPaths)
            {
                ...
                res = PdhAddEnglishCounter(_hQuery, counterPath, IntPtr.Zero, out counterHandle);
                if (res == 0)
                {
                ...
                else System.Diagnostics.Trace.WriteLine(Program.CurrentFormattedLocalDateTime() + ": Bad counter path encountered: " + counterPath);
            }
